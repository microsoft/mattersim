...
from mattersim.datasets.utils.ocp_graph_utils import radius_graph_pbc
from mattersim.datasets.utils.threebody_indices import compute_threebody as _compute_threebody
from mattersim.datasets.utils.threebody_indices_torch import (
    compute_threebody_torch as _compute_threebody_torch,
)
...
def compute_threebody_indices_torch(
    ...
    according to a threebody cutoff radius
    Args:
        edge_indices: torch.Tensor, (2, n_edges)
        distances: torch.Tensor, (n_edges,)
        num_atoms: torch.Tensor, (n_structures,)
        threebody_cutoff: float, threebody cutoff radius, default 4.0
    Returns:
        triple_bond_indices, n_triple_ij, n_triple_i, n_triple_s
    """
    ...
    if num_edges > 0 and threebody_cutoff is not None:
        ...
        original_index = torch.arange(num_edges, device=edge_indices.device)[valid_three_body]
    ...
    if num_edges > 0 and valid_edge_indices.shape[0] > 0:
        ...
        # No valid edges
        angle_indices = torch.zeros((0, 2), dtype=torch.long, device=edge_indices.device)
        if num_edges == 0:
            num_angles_per_edge = torch.zeros((0,), dtype=torch.int32, device=edge_indices.device)
        else:
            num_angles_per_edge = torch.zeros(
                (num_edges,), dtype=torch.int32, device=edge_indices.device
            )
        num_edges_per_atom = torch.zeros(
            (total_num_atoms,), dtype=torch.int32, device=edge_indices.device
        )
        num_angles_per_structure = torch.zeros(
            (num_atoms.shape[0],), dtype=torch.int32, device=edge_indices.device
        )
    return angle_indices, num_angles_per_edge, num_edges_per_atom, num_angles_per_structure



class BatchGraphConverter:
    """
    Convert batch of materials to graphs, with GPU support
    
    Args:
        model_type: Literal["m3gnet"], "m3gnet"
        twobody_cutoff: float cutoff for two body interactions, default 5.0 angstrom
        has_threebody: bool, whether to compute three body interactions, only for m3gnet, default True
        threebody_cutoff: float cutoff for three body interactions, default 4.0 angstrom
    """
    ...
    def convert(
        self,
        atoms_list: Atoms | Structure | list[Atoms | Structure],
        *,
        ...
        Convert the structure into graph
        Args:
            atoms_list: Atoms | Structure or list of Atoms | Structure
            energy: list of float, energies for each structure
            forces: list of np.ndarray, forces for each structure
            stresses: list of np.ndarray, stress for each structure
            max_natoms_per_batch: int, maximum number of atoms per batch
        """
        if isinstance(atoms_list, (Atoms, Structure)):
            atoms_list = [atoms_list]
        if isinstance(atoms_list[0], Structure):
            atoms_list = [AseAtomsAdaptor.get_atoms(structure) for structure in atoms_list]
        ...
        while pointer < len(atoms_list):
            ...
            cell = torch.zeros((0, 3, 3), dtype=torch.float32, device=self.device)
            atomic_numbers = torch.zeros((0,), dtype=torch.long, device=self.device)
            pbc = torch.zeros((0, 3), dtype=torch.bool, device=self.device)
            natoms_cumsum = 0
            num_graphs = 0
            while (
                pointer < len(atoms_list)
                and natoms_cumsum + len(atoms_list[pointer]) <= max_natoms_per_batch
            ):
                atoms = atoms_list[pointer].copy()
                atoms.wrap()
                natoms = torch.cat(
                    (natoms, torch.tensor([len(atoms)], dtype=torch.long, device=self.device)),
                    dim=0,
                )
                pos = torch.cat(
                    (
                ...
                cell = torch.cat(
                    (
                        cell,
                        torch.tensor(
                            np.array(atoms.cell), dtype=torch.float32, device=self.device
                        ).unsqueeze(0),
                    ),
                    dim=0,
                )
                ...
                pbc = torch.cat(
                    (
                        pbc,
                        torch.tensor(
                            np.array([atoms.pbc], dtype=bool), dtype=torch.bool, device=self.device
                        ),
                    ),
                    dim=0,
                )
            ...
            if num_graphs == 0:
                # Handle the case where a single graph exceeds max_natoms_per_batch anyway
                num_graphs = 1
                atoms = atoms_list[pointer].copy()
                atoms.wrap()
                natoms = torch.cat(
            ...
            edge_indices, offsets, num_edges, _, distances = radius_graph_pbc(
                pos=pos,
                pbc=pbc,
                cell=cell,
                natoms=natoms,
            ...
            # split the converted graphs into list of Data objects for each structure
            start_edge = 0
            start_atom = 0
            for i in range(num_graphs):
                ...
                graph["edge_index"] = (
                    edge_indices[:, start_edge : (start_edge + n_edges_i)] - start_atom
                )
                graph["distances"] = distances[start_edge : start_edge + n_edges_i]
                graph["pbc_offsets"] = offsets[start_edge : start_edge + n_edges_i]
                if self.has_threebody:
                    n_triple_ij_i = n_triple_ij[start_edge : start_edge + n_edges_i]
                    mask = (triple_bond_indices[:, 0] >= start_edge) & (
                        triple_bond_indices[:, 0] < start_edge + n_edges_i
                    )
                    triple_bond_indices_i = triple_bond_indices[mask, :] - start_edge
                ...
                if energy is not None and energy[pointer - num_graphs + i] is not None:
                    graph["energy"] = torch.tensor(
                        [energy[pointer - num_graphs + i]], dtype=torch.float32, device=self.device
                    )
                if forces is not None and forces[pointer - num_graphs + i] is not None:
                    graph["forces"] = torch.tensor(
                        forces[pointer - num_graphs + i], dtype=torch.float32, device=self.device
                    )
                if stresses is not None and stresses[pointer - num_graphs + i] is not None:
                    graph["stress"] = torch.tensor(
                        stresses[pointer - num_graphs + i], dtype=torch.float32, device=self.device
                    ).unsqueeze(0)
                graphs.append(Data(**graph).to(self.device))
